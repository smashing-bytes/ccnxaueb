...
{
	printf("[ccnd]line: %d consumed called for interest %s\n\n", __LINE__, pe->interest_msg);
    struct face *face = NULL;
...
{
	//printf("[ccnd] line: %d finalize_propagating called\n", __LINE__);
    struct ccnd_handle *h = hashtb_get_param(e->ht, NULL);
    consume(h, e->data);
}
    ...
    for (p = head->next; p != head; p = next)
    {
        ...
        if (p->interest_msg != NULL &&
                ((face == NULL && (f = face_from_faceid(h, p->faceid)) != NULL) ||
                 (face != NULL && p->faceid == face->faceid)) && p->pi_enabled == 0)
        {
                                             ...
                                             p->interest_msg, p->size, NULL))
            {
                ...
                matches += 1;
				printf("[ccnd]line: %d - Content matches interest:%s consume\n", __LINE__, p->interest_msg);
               
                /*Skip interest consumption*/
                if(p->pi_enabled)
					printf("[ccnd]line: %d - Skipped persistent interest consumption\n", __LINE__);
				else
					consume(h, p);
            }
        }
...
check_propagating(struct ccnd_handle *h)
{
    ...
    {
		//printf("Cleaning up, pe pi enabled = %d\n", pe->pi_enabled);
		//printf("Interest message %s\n", pe->interest_msg);
        if (pe->interest_msg == NULL /*&& pe->pi_enabled == 0*/)
        {
            if (pe->size == 0)
            {
				//res = ccn_parse_interest(pe->interest_msg, sizeof(pe->interest_msg), pi, comps);
				//if(pi->persistent)
				//	printf("Deleted persistent!\n");
                count += 1;
                hashtb_delete(e);
                printf("[ccnd]line: %d - Deleted expired interest message %s\n", __LINE__, pe->interest_msg);
                continue;
            }
    ...
    if (pe->usec <= 0)
    {
        if (h->debug & 2)
            ccnd_debug_ccnb(h, __LINE__, "interest_expiry",
                            face_from_faceid(h, pe->faceid),
                            pe->interest_msg, pe->size);
		printf("[ccnd]line: %d Interest expiry consume\n", __LINE__);
        consume(h, pe);
        reap_needed(h, 0);
        return(0);
    }
                   ...
                   struct nameprefix_entry *npe)
{
    ...
    if (pe != NULL && pe->interest_msg == NULL)
    {
        ...
        else
        {
			...
			{
				/*Respond time prediction does not matter for most Persistent Interests*/
				ccn_schedule_event(h->sched, usec, do_propagate, pe, 800000);
			}
                          ...
                          unsigned char *msg, size_t size)
{
    ...
    printf("[ccnd] Got an interest from face: %d\n", face->faceid);
    struct hashtb_enumerator ee;
    struct hashtb_enumerator *e = &ee;
    struct ccn_parsed_interest parsed_interest = {0};
    struct ccn_parsed_interest *pi = &parsed_interest;
                      ...
                      unsigned char *msg, size_t size, int pdu_ok)
{
    ...
    case CCN_DTAG_Interest:
        ...
        return;
    case CCN_DTAG_PersistentInterest:
        printf("[ccnd]Got a persistent interest!\n");
        process_incoming_interest(h, face, msg, size);
        return;
