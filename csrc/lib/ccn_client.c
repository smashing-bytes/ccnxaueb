                       ...
                       struct expressed_interest *dest)
{
    ...
    dest->persistent = 0; //Not persistent
    c->length = 0;
...
/**Overloaded --kinda*/
static void
ccn_construct_persistent_interest(struct ccn *h,
                       struct ccn_charbuf *name_prefix,
                       struct ccn_charbuf *interest_template,
                       struct expressed_interest *dest, unsigned char persistent)
{
    struct ccn_charbuf *c = h->interestbuf;
    size_t start;
    size_t size;
    ...
    /*Set struct persistent tag*/
    dest->persistent = persistent;
    c->length = 0;
    
    /*Append the "packet" special DTAG*/
    ccn_charbuf_append_tt(c, CCN_DTAG_PersistentInterest, CCN_DTAG);
    ccn_charbuf_append(c, name_prefix->buf, name_prefix->length);
    res = 0;
    if (interest_template != NULL) 
    {
    ...
    /*Actually send the interest*/
    replace_interest_msg(dest, (res >= 0 ? c : NULL));
}


/**
 * No go for overloading functions in C
 */
int
ccn_express_persistent_interest(struct ccn *h,
                     struct ccn_charbuf *namebuf,
                     struct ccn_closure *action,
                     struct ccn_charbuf *interest_template, unsigned char persistent)
{
    struct hashtb_enumerator ee;
    struct hashtb_enumerator *e = &ee;
    int res;
    ...
    /*Create prefix table if we don't have one*/
    if (h->interests_by_prefix == NULL) 
    {
    ...
    ccn_construct_persistent_interest(h, namebuf, interest_template, interest, persistent);
    if (interest->interest_msg == NULL) {
        free(interest);
        hashtb_end(e);
        return(-1);
...
{
	/*
	if(interest->persistent)
		printf("Persistent Interest sent\n\n");
	else
		printf("Normal Interest\n");
	*/
    int res;
    if (interest->magic != 0x7059e5f4) {
        ccn_gripe(interest);
